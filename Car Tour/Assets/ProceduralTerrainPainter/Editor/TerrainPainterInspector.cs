...
namespace sc.terrain.proceduralpainter
{
    ...
    public class TerrainPainterInspector : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            TerrainChangeListener.Current = script;
        ...
        private void DrawSettings()
        {
            ...
            if (terrains.arraySize == 0)
            {
                EditorGUILayout.HelpBox("Assigns terrains to paint on", MessageType.Info);
                EditorGUILayout.HelpBox("All existing terrain layers and painting will be cleared!", MessageType.Warning);
            }
            else
            {
                ...
                {
                    serializedObject.ApplyModifiedProperties();
                    script.AddOrRemoveTerrainListeners(autoRepaint.boolValue);
                }
        ...
        {
            if (m_LayerList != null) //Not yet initialized OnEnable
            {
                if (heatmapEnabled && m_LayerList.index > -1)
                {
        ...
        private void DrawLayers()
        {
            ...
            scrollview = EditorGUILayout.BeginScrollView(scrollview, GUILayout.MinHeight(kElementHeight + 17f), GUILayout.MaxHeight(kElementHeight * 4f + 17f));
            ...
            {
                if (script.layerSettings.Count > 0)
                {
                ...
                EditorGUI.BeginDisabledGroup(m_LayerList.index < 0);
            ...
            if (editLayerSettings && script.layerSettings.ElementAtOrDefault(m_LayerList.index) != null)
            {
                EditorGUILayout.Space();
        ...
        void DrawLayerModifierStack()
        {
            ...
            {
                EditorGUILayout.HelpBox("Base layer has no configurable options", MessageType.Info);
                if(layerSettings.arraySize == 1) EditorGUILayout.HelpBox("Add an additional terrain layer", MessageType.Info);
                return;
            }
            ...
            using (new EditorGUI.DisabledGroupScope(settings.enabled == false))
            {
                if (curModList != null)
                {
                    ...
                    if(curModList.index == -1) EditorGUILayout.HelpBox("Select a modifier from the stack to edit its settings", MessageType.Info);
        ...
        private void DrawModifierBackground(Rect rect, int index, bool isactive, bool isfocused)
        {
            ...
            if (isfocused) GUI.color = EditorGUIUtility.isProSkin ? Color.grey * 1.1f : Color.grey * 1.5f;
